...
get_candidate_moves(Turn,Board) ->
	...
	lists:usort(lists:foldl(fun({I,J},Acc)->
		case element(I,element(J,Board)) of
			...
			Color -> if abs(I-8)<Size andalso abs(J-8)<Size -> Acc; true -> get_arround({I,J},2,Board)++Acc end;
			_ -> Acc
		end
...
get_arround({X,Y},Size,Board) ->
	...
	end, [{X-Size+I,Y-Size+J} || I<-lists:seq(2*Size-1), J<-lists:seq(1,2*Size-1)]).
